...
class HourEntry {
  ...
  Map<String, dynamic> toJson() => {
        ...
        "actualDryLb": actualDryLb,
        "actualCookedLb": actualCookedLb,
        "minutes": minutes,
      };
...
class Calc {
  ...
  final double casesHour;
  final double dryHour;
  final double cookedHour;
  final double stopCases;

  Calc({
    required this.dryLbMin,
    required this.cookedLbMin,
    required this.casesHour,
    required this.dryHour,
    required this.cookedHour,
...
Calc calc(Product p) {
  ...
  final stopCases =
      (p.finishedCasesNeeded - flushCases).clamp(0, 999999).toDouble();
...
class Store extends ChangeNotifier {
  ...
  Future<void> load() async {
    ...
    if (pRaw == null) {
      ...
      products =
          (jsonDecode(pRaw) as List).map((e) => Product.fromJson(e)).toList();
    }

    if (hRaw != null) {
      hours =
          (jsonDecode(hRaw) as List).map((e) => HourEntry.fromJson(e)).toList();
    }
  ...
  Future<void> save() async {
    ...
    await sp.setString(
      "hours",
      jsonEncode(hours.map((e) => e.toJson()).toList()),
    );
  }
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        colorScheme:
            ColorScheme.fromSeed(seedColor: gold, brightness: Brightness.dark),
        useMaterial3: true,
      ),
      home: const Shell(),
    );
          ...
          bottomNavigationBar: NavigationBar(
            ...
            destinations: const [
              ...
              NavigationDestination(
                ...
                selectedIcon: Icon(Icons.bar_chart, color: gold),
                label: "Rate",
              ),
              NavigationDestination(
                icon: Icon(Icons.inventory_2_outlined),
                selectedIcon: Icon(Icons.inventory_2, color: gold),
                label: "Products",
              ),
              NavigationDestination(
                icon: Icon(Icons.psychology),
                selectedIcon: Icon(Icons.psychology, color: gold),
                label: "Advisor",
              ),
            ],
          ),
...
Widget logo([double h = 74]) {
  return Image.asset("assets/nates_logo.png", height: h, fit: BoxFit.contain);
}
...
Widget stat(String label, String value, [Color c = gold]) {
  return Expanded(
    child: Container(
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      children: [
        ...
        const Center(
          child: Text(
            "NATE'S PRODUCTION PRO",
            style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900),
          ),
        ),
        const Center(
          child: Text(
            "Pounds Per Hour + Blancher Tracking",
        ...
        ProCard(
          child: Column(
            ...
            children: [
              ...
              Text(
                p == null
                    ? "Create a product to begin"
                    : "Theoretical: ${f0(c!.cookedHour)} cooked lb/hr • ${f0(c.casesHour)} cases/hr",
        ...
        ProCard(
          child: Column(
            children: [
              const Text(
                "SHIFT TOTALS ENTERED",
                style: TextStyle(color: gold, fontWeight: FontWeight.w900),
              ),
              const SizedBox(height: 10),
              Row(
                children: [
                  stat("CASES", f0(totalCases)),
                  stat("DRY LB", f0(totalDry)),
                ],
              ),
            ],
          ),
...
class _CalculatorPageState extends State<CalculatorPage> {
  ...
  double v(TextEditingController c) => double.tryParse(c.text) ?? 0.0;
      ...
      children: [
        Header(
          ...
          action: IconButton(
            ...
            onPressed: () => widget.store.upsertProduct(current()),
          ),
        ),
        ProCard(
          child: Column(
        ...
        if (result != null)
          ProCard(
            child: Column(
              children: [
                const Text(
                  "STOP FEEDING WHEN PACKED CASES REACH",
...
class RateTrackerPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final totalDry = store.hours.fold<double>(0, (a, b) => a + b.actualDryLb);
    final totalCooked =
        store.hours.fold<double>(0, (a, b) => a + b.actualCookedLb);
    final totalMinutes = store.hours.fold<double>(0, (a, b) => a + b.minutes);

    final double actualCookedHr =
        totalMinutes > 0 ? totalCooked / (totalMinutes / 60) : 0.0;

    final double actualCasesHr =
        totalMinutes > 0 ? totalCases / (totalMinutes / 60) : 0.0;

    final double efficiency = c.cookedHour > 0
        ? (actualCookedHr / c.cookedHour * 100).clamp(0, 999).toDouble()
        : 0.0;

    final expectedCasesByTime = c.casesHour * (totalMinutes / 60);
    final double pace = totalCases - expectedCasesByTime;
      ...
      children: [
        Header(
          ...
          action: IconButton(
            ...
            onPressed: () => openHourEditor(context, store, p),
          ),
        ),
        ProCard(
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  stat("COOKED LB/HR", f0(c.cookedHour)),
                  stat("CASES/HR", f0(c.casesHour)),
                ],
              ),
              Row(
                children: [
                  stat("DRY LB/HR", f0(c.dryHour)),
                  stat("FEED SEC", f(p.secondsPerFeed)),
                ],
              ),
            ],
          ),
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  stat("ACTUAL CASES/HR", f0(actualCasesHr)),
                ],
              ),
              Row(
                children: [
              ...
              Text(
                pace >= 0
                    ? "Ahead of theoretical pace"
                    : "Behind theoretical pace",
                style: TextStyle(
                  color: pace >= 0 ? green : red,
                  fontWeight: FontWeight.w900,
                ),
              ),
            ],
        ...
        ProCard(
          child: Column(
            children: [
              ...
              Row(
                children: [
                  stat("COOKED LB", f0(totalCooked)),
                  stat("HOURS", f(totalMinutes / 60)),
                ],
              ),
              const SizedBox(height: 8),
              OutlinedButton.icon(
                onPressed: () => confirmClear(context, store),
        ...
        if (store.hours.isEmpty)
          const ProCard(
            child: Text("No hourly entries yet. Tap + to add an hour or time block."),
          ),
        ...store.hours.map((h) {
          final double hCookedHr =
              h.minutes > 0 ? h.actualCookedLb / (h.minutes / 60) : 0.0;

          final double hCasesHr =
              h.minutes > 0 ? h.actualCases / (h.minutes / 60) : 0.0;

          final double hEfficiency = c.cookedHour > 0
              ? (hCookedHr / c.cookedHour * 100).clamp(0, 999).toDouble()
              : 0.0;
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        "${f0(h.actualCookedLb)} cooked lb • ${f0(h.actualCases)} cases",
                        style: const TextStyle(color: muted),
                      ),
                      Text(
                        "${f0(hCookedHr)} lb/hr • ${f0(hCasesHr)} cases/hr • ${f0(hEfficiency)}%",
                        style: const TextStyle(color: muted),
                      ),
                    ],
                  ),
      ...
      builder: (_) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            ...
            child: const Text("Clear", style: TextStyle(color: red)),
          ),
        ],
      ),
    );
  ...
  void openHourEditor(BuildContext context, Store store, Product p) {
    ...
    showModalBottomSheet(
      ...
      builder: (_) => HourEditor(store: store, calcData: c, product: p),
    );
  }
}
...
class HourEditor extends StatefulWidget {
  ...
  @override
  State<HourEditor> createState() => _HourEditorState();
}
...
class _HourEditorState extends State<HourEditor> {
  ...
  late TextEditingController cases;
  late TextEditingController dry;
  late TextEditingController cooked;
  late TextEditingController minutes;
  ...
  void fillTheoretical() {
    final mins = v(minutes) <= 0 ? 60.0 : v(minutes);
      ...
      child: SingleChildScrollView(
        child: Column(
          children: [
            ...
            const SizedBox(height: 8),
            const Text(
              "Enter what actually happened for an hour or any time block.",
              style: TextStyle(color: muted),
            ...
            const SizedBox(height: 8),
            OutlinedButton.icon(
              onPressed: fillTheoretical,
              icon: const Icon(Icons.auto_fix_high, color: gold),
              label: const Text(
                "Fill with theoretical rate",
                style: TextStyle(color: gold),
              ),
            ),
            const SizedBox(height: 10),
            GoldButton("SAVE ENTRY", Icons.save, () {
              widget.store.addHour(
                HourEntry(
                  id: DateTime.now().millisecondsSinceEpoch.toString(),
                  ...
                  minutes: v(minutes) <= 0 ? 60.0 : v(minutes),
                ),
              );
              Navigator.pop(context);
            }),
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        "${f(p.dryWeightPerFeed)} lb every ${f(p.secondsPerFeed)} sec • yield ${f(p.growthRate)}",
                        style: const TextStyle(color: muted),
                      ),
                    ],
                  ),
...
class AdvisorPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final double actualHr =
        totalMinutes > 0 ? totalCooked / (totalMinutes / 60) : 0.0;

    final double efficiency =
        c.cookedHour > 0 ? (actualHr / c.cookedHour * 100).toDouble() : 0.0;
          ...
          child: Column(
            ...
            children: [
              ...
              advice(
                "Pounds/hour",
                actualHr == 0
                    ? "Add an hourly entry to compare actual pounds/hour against theoretical."
                    : "Actual cooked pounds/hour is ${f0(actualHr)} versus theoretical ${f0(c.cookedHour)}.",
              ),
              advice(
                "Efficiency",
                efficiency == 0
                    ? "No efficiency yet."
                    : efficiency >= 95
                        ? "Efficiency is strong at ${f0(efficiency)}%. Keep current pace."
                        : efficiency >= 85
                            ? "Efficiency is ${f0(efficiency)}%. Check feed timing, line delays, and pack-out speed."
                            : "Efficiency is low at ${f0(efficiency)}%. Check for downtime, slow feeding, or pack-out bottleneck.",
              ),
              advice(
                "Cases",
                "Total tracked cases this shift: ${f0(totalCases)}.",
              ),
            ],
          ),
        ),
  ...
  Widget advice(String title, String body) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: navy2,
        borderRadius: BorderRadius.circular(14),
        border: const Border(left: BorderSide(color: gold, width: 5)),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(title, style: const TextStyle(color: gold, fontWeight: FontWeight.w900)),
